#' @export
plot_stock_status_determination <- function(em_names, col, casedir, input_list){
  comparison_var <- c("fratio", "ssbratio")
  comparison_id <- which(names(om_list) %in% comparison_var)

  overfishing_data <- list()
  overfishing_accuracy <- matrix(NA, nrow=length(input_list$year), ncol=length(em_names))
  for (k in 1:length(em_names)){
    overfishing_data[[k]] <- matrix(NA, nrow=length(input_list$year), ncol=keep_sim_num)
    for(j in 1:keep_sim_num){
      for (i in 1:length(input_list$year)){
  ...
  rownames(overfishing_accuracy) <- input_list$year
  write.csv(overfishing_accuracy, file=file.path(casedir, "output", "overfishing_accuracy.csv"))

  overfished_data <- list()
  overfished_accuracy <- matrix(NA, nrow=length(input_list$year), ncol=length(em_names))
  for (k in 1:length(em_names)){
    overfished_data[[k]] <- matrix(NA, nrow=length(input_list$year), ncol=keep_sim_num)
    for(j in 1:keep_sim_num){
      for (i in 1:length(input_list$year)){
        if((em_list[[k]][[comparison_id[1]]][i,j]<1 & om_list$fratio[i,j]<1) | (em_list[[k]][[comparison_id[1]]][i,j]>1 & om_list$fratio[i,j]>1)){
  ...
  rownames(overfished_accuracy) <- input_list$year
