...
plot_msy_re <- function(em_names, col, casedir=NULL){
  ...
  msy_sim_boxplot <- boxplot(msy_re[,seq(dim(msy_re)[2],1)],
                             ...
                             at=c(1:length(em_names),  (length(em_names)+2):(length(em_names)*2+1), (length(em_names)*2+3):(length(em_names)*3+2),(length(em_names)*3+2):(length(em_names)*4+1)),
                             xlab="Relative Error",
                             ylim=c(min(-0.5,  min(msy_re[is.finite(msy_re)])), max(0.5,  max(msy_re[is.finite(msy_re)]))),
                             outline=FALSE)
