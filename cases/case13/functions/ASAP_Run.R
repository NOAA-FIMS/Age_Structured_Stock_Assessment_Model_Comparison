ASAP_Run <- function(maindir=maindir, subdir="ASAP", om_sim_num=1){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if(modify_input == "partial") {
      ...
      asap_input$dat$IAA_mats[[1]] <- cbind(om_input$yr, em_input$survey.obs$survey1, rep(em_input$cv.survey$survey1, times=length(em_input$survey.obs$survey1)), em_input$survey.age.obs$survey1, rep(em_input$n.survey$survey1, times=length(em_input$survey.obs$survey1)))
