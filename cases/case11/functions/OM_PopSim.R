...
popsim<-function(x){
  ...
  fishery_data_year=x$fishery_data_year; survey_data_year=x$survey_data_year
  ...
  invisible(sapply(1:length(L.mt), function(x) L.mt[[x]] <<- rep(NA,nyr)))
  invisible(sapply(1:length(L.knum), function(x) L.knum[[x]] <<- rep(NA,nyr)))

  L.age<-vector(mode="list", length=fleet_num)
  names(L.age) <- paste("fleet", 1:fleet_num, sep="")
  invisible(sapply(1:length(L.age), function(x) L.age[[x]] <<- matrix(NA, nrow=nyr, ncol=nages)))

  N.age<-FAA<-matrix(NA, nrow=nyr, ncol=nages)
  ...
  L.age$fleet1[-c(catch_age_data_year),]
  L.knum$fleet1[-c(catch_data_year)]
  L.mt$fleet1[-c(catch_data_year)]
