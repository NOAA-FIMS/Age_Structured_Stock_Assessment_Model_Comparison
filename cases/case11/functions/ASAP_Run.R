ASAP_Run <- function(maindir=maindir, subdir="ASAP", om_sim_num=1){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if(modify_input == "partial") {
      asap_input$dat$n_years <- om_input$nyr
      asap_input$dat$year1 <- om_input$yr[1]
      asap_input$dat$CAA_mats[[1]] <- cbind(rbind(matrix(rep(rep(0, om_input$nages), times=om_input$nyr-length(em_input$catch_age_data_year)), nrow = om_input$nyr-length(em_input$catch_age_data_year), byrow = T),em_input$L.age.obs$fleet1), em_input$L.obs$fleet1)
      asap_input$dat$catch_cv <- cbind(rep(em_input$cv.L$fleet1, times=length(em_input$L.obs$fleet1)))

      asap_input$dat$IAA_mats[[1]] <- rbind(cbind(om_input$yr[-em_input$survey_age_data_year],
            rep(-999, length(om_input$yr[-em_input$survey_age_data_year])),
            rep(0, length(om_input$yr[-em_input$survey_age_data_year])),
            matrix(rep(rep(0, om_input$nages), times=om_input$nyr-length(em_input$survey_age_data_year)), nrow = om_input$nyr-length(em_input$survey_age_data_year), byrow = T),
            rep(0, length(om_input$yr[-em_input$survey_age_data_year]))),
             cbind(em_input$survey_age_data_year,
                   em_input$survey.obs$survey1,
                   rep(em_input$cv.survey$survey1, times=length(em_input$survey.obs$survey1)),
                   em_input$survey.age.obs$survey1,
                   rep(em_input$n.survey$survey1, times=length(em_input$survey.obs$survey1))))

      asap_input$dat$catch_Neff <- cbind(rep(em_input$n.L$fleet1, length(em_input$L.obs$fleet1)))
      asap_input$dat$F1_ini <- om_output$f[1]
      asap_input$dat$q_ini <- em_input$survey_q$survey1*1000
  ...
  for (om_sim in 1:om_sim_num){
    ...
    system(paste("ASAP3.exe asap3.DAT", sep = ""), show.output.on.console = FALSE)
  }
}