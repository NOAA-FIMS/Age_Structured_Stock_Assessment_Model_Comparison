BAM_Run <- function(maindir=maindir, subdir="BAM", om_sim_num=1){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if (modify_input == "partial"){
      ...
      dat1.survey=list(nyr=length(em_input$survey_index_year),
                       yrs=em_input$survey_index_year,
                       vals.obs=em_input$survey.obs$survey1,
                       cv=rep(em_input$cv.survey$survey1, length(em_input$survey_index_year)),
                       nyr.ages=length(em_input$survey_age_data_year),
                       yrs.age=em_input$survey_age_data_year,
                       nsamp=rep(em_input$n.survey$survey1, length(em_input$survey_age_data_year)),
                       nfish=rep(em_input$n.survey$survey1, length(em_input$survey_age_data_year)),
                       acomp=em_input$survey.age.obs$survey1)

      dat1.L=list(styr=em_input$catch_data_year[1],
                  endyr=em_input$catch_data_year[length(em_input$catch_data_year)],
                  vals.obs=em_input$L.obs$fleet1,
                  cv=rep(em_input$cv.L$fleet1, length(em_input$catch_data_year)),
                  nyr.ages=length(em_input$catch_age_data_year),
                  yrs.age=em_input$catch_age_data_year,
                  nsamp=rep(em_input$n.L$fleet1, length(em_input$catch_age_data_year)),
                  nfish=rep(em_input$n.L$fleet1, length(em_input$catch_age_data_year)),
