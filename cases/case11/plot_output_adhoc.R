...
for(om_sim in 1:om_sim_num){
  ...
  amak_msy_adhoc=msy_calcs(steep=om_input$h, R0=exp(amak_std$value[which(amak_std$name=="log_Rzero")]), M=matrix(rep(om_input$M.age, times=om_input$nyr), nrow=om_input$nyr, byrow=T), wgt=om_input$W.mt, prop.f=om_input$proportion.female, selL=amak_output$sel_fsh_1[1,3:ncol(amak_output$sel_fsh_1)], selD=rep(0,om_input$nages), selZ=amak_output$sel_fsh_1[1,3:ncol(amak_output$sel_fsh_1)], mat.f=om_input$mat.age, mat.m=NULL, BC=exp(amak_output$sigmar^2/2), maxF=1.0, step=0.001)
...
for(om_sim in 1:om_sim_num){
  ...
  asap_msy_adhoc <- msy_calcs(steep=om_input$h, R0=asap_output$SR.parms$SR.R0*1000, M=matrix(rep(om_input$M.age, times=om_input$nyr), nrow=om_input$nyr, byrow=T), wgt=om_input$W.mt, prop.f=om_input$proportion.female, selL=asap_output$fleet.sel.mats$sel.m.fleet1[1,], selD=rep(0,om_input$nages), selZ=asap_output$fleet.sel.mats$sel.m.fleet1[1,], mat.f=om_input$mat.age, mat.m=NULL, BC=exp(log(asap_output$control.parms$recruit.cv[1]^2+1)/2), maxF=1.0, step=0.001)
