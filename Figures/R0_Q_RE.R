...
R0_RE <- matrix(NA, nrow=length(maindir_list), ncol=4)
Q_RE <- matrix(NA, nrow=length(maindir_list), ncol=4)

#### R0 and Q relative error table ####
...
maindir_list  <- c("C:/Users/bai.li/Desktop/mcp_results_r/cases/case2_1", 
                   "C:/Users/bai.li/Desktop/mcp_results_r/cases/case2_1_F0_0.01_1yr") 
R0_RE <- matrix(NA, nrow=length(maindir_list), ncol=4)
Q_RE <- matrix(NA, nrow=length(maindir_list), ncol=4)
...
boxplot_data <- matrix(NA, nrow=100, ncol=8)
sapply(1:8, function(x) {
  if (x<5) boxplot_data[,x] <<- estimates_cases[[1]][[x+1]]$Rzero
  else boxplot_data[,x] <<- estimates_cases[[2]][[x-3]]$Rzero
})

jpeg(file="C:/Users/bai.li/Desktop/mcp_results_r/cases/manuscript_figures/Rzero_AMAK.jpg", width=100, height=100, units="mm", res=300)
boxplot(boxplot_data, beside=T, axes=F, ylab="R0 (x1000 fish)", col=rep(c("orange", "green", "red", "deepskyblue"), times=2), pch=16, ylim=range(boxplot_data), cex=0.6)
box()
axis(1, at=1:8, labels = rep(c("AMAK", "ASAP", "BAM", "SS"), times=2), las=2)
axis(2)
