SS_Run <- function(maindir=maindir, subdir="SS", om_sim_num=1){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if(modify_input == "partial") {
      ss_data$catch <- as.data.frame(cbind(om_input$yr, rep(1, times=om_input$nyr), rep(1, times=om_input$nyr), em_input$L.obs$fleet1, rep(sqrt(log(1+em_input$cv.L$fleet1^2)), times=length(em_input$L.obs$fleet1))))
