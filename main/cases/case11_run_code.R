...
om_sim_num <- 160 # total iterations per case
...
for (om_sim in 1:om_sim_num){
  ...
  if(!file.exists(file.path(maindir, "output", subdir, paste("s", om_sim, sep=""), "admodel.hes"))) {
    convergence_measures$positive_hessian[om_sim, 2] <- 0
    convergence_measures$gradient_0.0001[om_sim, 2]<- NA
    convergence_measures$gradient_0.001[om_sim, 2]<- NA
    convergence_measures$gradient_0.01[om_sim, 2]<- NA
    convergence_measures$gradient[om_sim, 2]<- NA
  } else{
    temp <- getADMBHessian(File=file.path(maindir, "output", subdir, paste("s", om_sim, sep="")), FileName="admodel.hes")$hes
...
good_gradient_percentage[,1] <- length(which(convergence_measures$gradient[,1]<=0.002))/om_sim_num*100
good_gradient_percentage[,2] <- length(which(convergence_measures$gradient[,2]<=0.002))/om_sim_num*100
good_gradient_percentage[,3] <- length(which(convergence_measures$gradient[,3]<=0.002))/om_sim_num*100
good_gradient_percentage[,4] <- length(which(convergence_measures$gradient[,4]<=0.002))/om_sim_num*100
