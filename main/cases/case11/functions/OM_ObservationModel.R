...
#### Sampling of age comps (rows=years, columns=ages) ####
  ...
  invisible(sapply(1:length(L.age.obs), function(x) L.age.obs[[x]] <<- matrix(NA, nrow=length(om_input$catch_age_data_year), ncol=nages)))

  invisible(sapply(1:fleet_num, function(x){
    for (i in 1:length(om_input$catch_age_data_year)){
  ...
  invisible(sapply(1:length(survey.age.obs), function(x) survey.age.obs[[x]] <<- matrix(NA, nrow=length(om_input$catch_age_data_year), ncol=nages)))
