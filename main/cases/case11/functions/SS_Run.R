  ...
  for (om_sim in 1:om_sim_num){
    ...
    if(modify_input == "partial") {
      ss_data$catch <- as.data.frame(rbind(c(-999, 1, 1, em_input$L.obs$fleet1[1]/om_input$nages, sqrt(log(1+em_input$cv.L$fleet1^2))),
                                           cbind(em_input$catch_data_year,
      ...
      ss_data$CPUE <- as.data.frame(cbind(em_input$survey_index_year,
                                          rep(1, times=length(em_input$survey_index_year)),
                                          rep(2, times=length(em_input$survey_index_year)),
                                          em_input$survey.obs$survey1,
                                          rep(sqrt(log(1+em_input$cv.survey$survey1^2)), times=length(em_input$survey_index_year))))
      names(ss_data$CPUE) <- c("year", "seas", "index", "obs", "se_log")

      #agecomp_colnames <- colnames(ss_data$agecomp)
      ss_data$agecomp[, 9:ncol(ss_data$agecomp)] <- rbind(
            cbind(rep(em_input$n.L$fleet1, length(em_input$catch_age_data_year)),
            em_input$L.age.obs$fleet1),

            cbind(rep(em_input$n.survey$survey1, length(em_input$survey_age_data_year)),
