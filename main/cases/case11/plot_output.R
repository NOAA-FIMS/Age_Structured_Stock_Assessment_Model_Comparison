...
for(om_sim in 1:om_sim_num){
  ...
  om_survey[om_input$survey_index_year,om_sim] <- om_output$survey_index$survey1
  ...
  om_survey_err[om_input$survey_index_year,om_sim] <- em_input$survey.obs$survey1
}
...
for(om_sim in 1:om_sim_num){
  ...
  amak_survey[om_input$survey_index_year,om_sim] <- as.numeric(amak_output$Obs_Survey_1[om_input$survey_index_year,3])
...
for(om_sim in 1:om_sim_num){
  ...
  asap_survey[om_input$survey_index_year,om_sim] <- asap_output$index.pred$ind01
...
for(om_sim in 1:om_sim_num){
  ...
  bam_survey[om_input$survey_index_year,om_sim] <- bam_output$t.series$U.survey1.pr[om_input$survey_index_year]
...
for(om_sim in 1:om_sim_num){
  ...
  ss_survey[om_input$survey_index_year,om_sim] <- ss_output$cpue$Exp
  ss_msy[, om_sim] <- msy_std[15, "value"]
  ss_fmsy[, om_sim] <- round(msy_std[14, "value"], digits = 2)
...
for (j in figure_id){
  ...
  for(i in 1:length(plot_data_id)){
    if(i==length(plot_data_id)){
      ylim=range(om_list[[plot_data_id[i]]][om_input$survey_index_year,j], amak_list[[plot_data_id[i]]][om_input$survey_index_year,j], asap_list[[plot_data_id[i]]][om_input$survey_index_year,j], bam_list[[plot_data_id[i]]][om_input$survey_index_year,j], ss_list[[plot_data_id[i]]][om_input$survey_index_year,j])
    plot(om_input$survey_index_year, om_list[[plot_data_id[i]]][om_input$survey_index_year,j], pch=19, col=col[1], cex=0.7, ylim=ylim, axes=F, xlab="", ylab="")
    lines(om_input$survey_index_year, amak_list[[plot_data_id[i]]][om_input$survey_index_year,j], typ="l", col=col[2], lwd=1.5)
    lines(om_input$survey_index_year, asap_list[[plot_data_id[i]]][om_input$survey_index_year,j], typ="l", col=col[3], lwd=1.5)
    lines(om_input$survey_index_year, bam_list[[plot_data_id[i]]][om_input$survey_index_year,j], typ="l", col=col[4], lwd=1.5)
...
for (i in 1:length(plot_data_id)){
  ...
  for(k in 1:4){
    ...
    re_quantile <- sapply(1:om_input$nyr, function(x) quantile(temp[x,], c(0.1, 0.25, 0.5, 0.75, 0.9), na.rm = T))

    plot(re_quantile[3,], type="l", col="white", ylim=c(-max(abs(re_quantile), 0.3, na.rm=T), max(abs(re_quantile), 0.3, na.rm=T)), axes=F, xlab="", ylab="Relative Error", panel.first=grid(lty=1))
...
for (i in 1:length(plot_data_id)){
  ...
  ylim=range(om_list[[plot_data_id[i]]], unlist(temp_list), na.rm = T)
...
for(k in 1:5){
  for (om_sim in 1:om_sim_num){
    if(k==1){
      load(file.path(maindir, "output", "OM", paste("OM", keep_sim_id[om_sim], ".RData", sep="")))
      temp$Rzero[om_sim] <- om_output$msy$R_eq[1]/1000
      temp$Q[om_sim] <- om_output$survey_q$survey1
    }
    if(k==2){
      setwd(file.path(maindir,"output", "AMAK", paste("s", keep_sim_id[om_sim], sep="")))
    ...
    if(k==4){
      bam_output <- dget(file.path(maindir, "output", "BAM", paste("s", keep_sim_id[om_sim], sep=""), "bam-sim.rdat"))
      temp$Rzero[om_sim] <- bam_output$parms$R.virgin.bc/1000
      temp$Q[om_sim] <- bam_output$parms$q.survey1
    }
    if(k==5){
      ss_output <- SS_output(dir=file.path(maindir, "output", "SS", paste("s", keep_sim_id[om_sim], sep="")), ncols = 300, verbose=F, printstats=F)
...
col= c("orange", "green", "red", "deepskyblue3")
jpeg(file=file.path(maindir, subdir, "Rzero_sim_adjusted.jpg"), width=150, height=120, units="mm", res=300)
temp <- matrix(NA, nrow=om_sim_num, ncol=5)
for(k in 1:5){
  temp[,k] <- parameter_val_list[[k]][,1]
}
...
for (i in 1:length(plot_data_id)){
  ...
  par(mar=c(0.7, 4, 0.2, 0.2), mfrow=c(2,2), oma = c(4, 4, 0.2, 0.2))
  temp_list <- list(amak_list[[plot_data_id[i]]], asap_list[[plot_data_id[i]]], bam_list[[plot_data_id[i]]], ss_list[[plot_data_id[i]]])
  ylim=range(om_list[[plot_data_id[i]+8]], unlist(temp_list), na.rm = T)
  for(k in 1:length(temp_list)){
