...
om_input <- list(fleet_num=fleet_num, survey_num=survey_num, nyr=nyr, yr=yr, ages=ages, nages=nages, cv.L=cv.L, cv.survey=cv.survey, n.L=n.L, n.survey=n.survey, logR_sd=logR_sd, logf_sd=logf_sd, R0=R0, h=h, M=M, Linf=Linf, K=K, a0=a0, a.lw=a.lw, b.lw=b.lw, A50.mat=A50.mat, slope.mat=slope.mat, sel_fleet=sel_fleet, sel_survey=sel_survey, len=len, W.kg=W.kg, W.mt=W.mt, M.age=M.age, mat.age=mat.age, proportion.female=proportion.female, selex_fleet=selex_fleet, selex_survey=selex_survey, N.pr0=N.pr0, Phi.0=Phi.0, logR.resid=logR.resid, logf.resid=logf.resid, f=f, om_bias_cor=om_bias_cor, catch_data_year=catch_data_year, catch_age_data_year=catch_age_data_year, survey_index_year=survey_index_year, survey_age_data_year=survey_age_data_year)

#### Simulate the stock dynamics ####
input1<-list(nyr=nyr, f=f, ages=ages, nages=nages, R0=R0, h=h, Phi.0=Phi.0, M.age=M.age, W.mt=W.mt, mat.age=mat.age, prop.f=proportion.female, selex_fleet=selex_fleet, logR.resid=logR.resid, om_bias_cor=om_bias_cor, catch_data_year=catch_data_year, catch_age_data_year=catch_age_data_year, survey_index_year=survey_index_year, survey_age_data_year=survey_age_data_year)
...
invisible(sapply(1:length(survey_age_comp), function(x) {
  survey_age_comp[[x]] <<- om_output$N.age[survey_age_data_year,]%*%diag(selex_survey[[x]])
