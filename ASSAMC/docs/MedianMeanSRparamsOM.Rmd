...
### 1.2 Median-unbiased stock-recruit paramters
...
* Forward projection without fishin mortality
...
* Life history parameter could be found in Appendix
...
for(i in 1:nrow(forloop_id)){
  ...
  for(j in 1:updated_input$om_sim_num){
    ...
    depletion[j,] <- om_output$SSB/(om_input$R0*om_input$Phi.0)
  }
...
for(i in 1:length(meanh_vec)){
  plot(apply(depletion_list[[which(forloop_id$meanh_vec==meanh_vec[i])[1]]], 2, mean), ylim=c(-1,4), xlab="Year", ylab="Mean SSB/SSB0", type="l", col=color[1])
...
for(i in 1:length(medianh_vec)){
  plot(apply(depletion_list[[which(forloop_id$medianh_vec==medianh_vec[i])[1]]], 2, mean), ylim=c(-1,4), xlab="Year", ylab="Mean SSB/SSB0", type="l", col=color[1])
  for(j in 1:length(logR_sd_vec)){
...
### 2.3 OM_Median with converted median-unbiased stock-recruit paramters from means
...
### 2.4 OM_Mean with converted mean-unbiased stock-recruit paramters from medians
...
### 3.3 OM_Median with converted median-unbiased stock-recruit paramters from means
...
### 3.4 OM_Mean with converted mean-unbiased stock-recruit paramters from medians
