run_bam <- function(maindir=maindir, subdir="BAM", om_sim_num=NULL, casedir=casedir){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if (modify_input == "partial"){
      ...
      BAM.write.dat(fname=file.path(casedir, "output", subdir, paste("s", om_sim, sep=""), "BAM-Sim.dat"), nyr=om_input$nyr, nages=om_input$nages, dat.survey=dat1.survey, dat.L=dat1.L, parms=bam_parms, a.lw=om_input$a.lw, b.lw=om_input$b.lw, prop.f=om_input$proportion.female, mat.age=om_input$mat.age, M.age=om_input$M.age, em_bias_cor=em_bias_cor)
    }
  }
